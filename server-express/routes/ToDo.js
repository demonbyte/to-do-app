...
router.post("/", async function (req, res) {
  console.log("Inhell oooooooooooooooooooooooooooo POSTsss /post handler");
  const post = new Post({
    // id:req.payload.id,
...
router.delete("/:id", async function (req, res) {
  ...
  try {
    ...
    if (deletedPost) {
      ...
      // Return the deleted post details in the response
      return res.status(200).json({
        id: deletedPost._id,
        title: deletedPost.title,
        description: deletedPost.description,
        author: deletedPost.author,
        complete: deletedPost.complete,
        dateCreated: deletedPost.dateCreated,
        dateCompleted: deletedPost.dateCompleted,
      });
    } else {
...
router.patch('/:id', async (req, res) => {
  ...
  const newTodo = await Post.findOneAndUpdate(
    { _id: id },
